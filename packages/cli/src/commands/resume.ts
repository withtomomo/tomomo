...
export const resumeCommand = new Command("resume")
  .description("Resume last session with an agent")
  .argument("<agent>", "Agent ID")
  .argument("[path]", "Project directory", ".")
  .option("--json", "Output as JSON")
  .action(
    async (agentId: string, path: string, options: { json?: boolean }) => {
      ...
      try {
        const result = await launchAgent({ agentId, projectDir, resume: true });
        if (options.json) {
          console.log(
            JSON.stringify(
